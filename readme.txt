...
* Ant and Jython require the following additional Apache (www.apache.org) tools:
  * ant-contrib (http://ant-contrib.sourceforge.net/) - Download the binary zip from https://sourceforge.net/projects/ant-contrib/files/ant-contrib/1.0b3/ant-contrib-1.0b3-bin.zip/download,  You must extract the ant-contrib-1.0b3.jar file and placein in the Ant lib folder (e.g. C:\Tools\apache-ant-1.9.4\lib).
...
This remainder of the process takes the sermon MP3, updates the HTML file, and FTPs the files to the server. For this process you need to have:
...
1. Never edit the cbcmedia.html file manually unless you know what you are doing.  The ant regex processing depends on the format I've established; so, if you mess anything up it will break the process.
 ...
 * The FTP process should not fail (the CBC-Sermon.bat file pre-checks this), but in the unlikely event it does fail this is typical of the error you might see:
    BUILD FAILED
